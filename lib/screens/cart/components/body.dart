...
import 'package:e_commerce_app_flutter/services/data_streams/cart_annonces_stream.dart';
...
class _BodyState extends State<Body> {
  final CartAnnoncesStream cartAnnoncesStream = CartAnnoncesStream();
  PersistentBottomSheetController bottomSheetHandler;
  @override
  void initState() {
    super.initState();
    cartAnnoncesStream.init();
  }

  @override
  void dispose() {
    super.dispose();
    cartAnnoncesStream.dispose();
  }
  ...
  Future<void> refreshPage() {
    cartAnnoncesStream.reload();
    return Future<void>.value();
  }

  Widget buildCartItemsList() {
    return StreamBuilder<List<String>>(
      stream: cartAnnoncesStream.stream,
            ...
            if (direction == DismissDirection.startToEnd) {
              ...
              try {
                result = await UserDatabaseHelper()
                    .removeAnnonceFromCart(cartItemId);
                if (result == true) {
                  snackbarMessage = "Item removed from cart successfully";
                  await refreshPage();
                } else {
                  throw "Coulnd't remove item from cart due to unknown reason";
                }
  ...
  Widget buildCartItem(String cartItemId, int index) {
    return Container(
      ...
      child: FutureBuilder<Annonce>(
        future: AnnonceDatabaseHelper().getAnnonceWithID(cartItemId),
        builder: (context, snapshot) {
          if (snapshot.hasData) {
  ...
  Future<void> checkoutButtonCallback() async {
    ...
    final confirmation = await showConfirmationDialog(
      context,
      "This is just a Project App so, no actual Payment Interface is available.\nDo you want to proceed to My orders ?",
    );
    ...
    orderFuture.then((orderedProductsUid) async {
      if (orderedProductsUid != null) {
        ...
        List<OrderedAnnonce> orderedProducts = orderedProductsUid
            .map((e) => OrderedAnnonce(null,
                annonceUid: e, orderDate: formatedDateTime))
            .toList();
        ...
        try {
          ...
          if (addedProductsToMyProducts) {
            snackbarmMessage = "Items ordered Successfully";
          } else {
            throw "Could not order items due to unknown issue";
          }
        } on FirebaseException catch (e) {
          Logger().e(e.toString());
